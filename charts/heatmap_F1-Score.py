...
if mode == 'lora':
   ...
   name_table= "Bản đồ nhiệt chỉ số F1-Score theo từng lớp bệnh (LoRA)"
else:
   model_name_file= ['full_efficientnet', 'full_resnet', 'full_densenet', 'full_mobilenet', 'full_googlenet', 'full_vgg16']
   name_table= "Bản đồ nhiệt chỉ số F1-Score theo từng lớp bệnh (Full)"
...
for display_name, file_name in zip(model_name, model_name_file):
   ...
   auc = df_csv[df_csv["Class"].isin(classes)]["F1-Score"].tolist()
   
   if len(auc) == len(classes):
      data[display_name] = auc
   else:
      print(f"Cảnh báo: Số lượng F1-Score ({len(auc)}) không khớp với số lượng lớp ({len(classes)}) cho {display_name}")
